...
Route::group(['middleware' => ['domain']], function () {
	...
	// sasapay
	Route::any('payment-success', 'Front\SasaPayController@success')->name('payment.success');  
	...
	// sasapay gateway
	Route::match(['get', 'post'], 'payment/sasapay/page', 'Front\SasaPayController@beforePayment')->name('payment.sasapay.beforePayment');
	Route::post('payment/sasapay', 'Front\SasaPayController@createPayment')->name('payment.sasapay.createPayment');
    ...
    Route::get('get/products/{productType}', 'Front\CategoryController@getProducts')->name('getProducts');
    Route::post('products/filter', 'Front\CategoryController@filterProducts')->name('filterProducts');
...
Route::group(['middleware' => ['domain', 'webAuth']], function () {
	 ...
	 */
	Route::post('booking/checkProductAvailibility', 'Front\Booking\ProductBookingController@checkProductAvailibility')->name('product-booking.checkProductAvailibility');   # update all product actions
	Route::get('curl-sasa', function(){

		$url = 'https://sandbox.sasapay.app/api/v1/auth/token/?grant_type=client_credentials';
		  $requestBody = array(
			   'client_id' => '3QRkR679zqkCoraNroFrDjK8Ipv8HhQKEl1tmkA6',
			  'client_secret' => 'CQ3N3RZwhmtsOFQ59SYSxHnlyn8SwVTztnsjkQzhd5L3VtVCCbAMdAKyHN7kKP16cKIxCxuvezb1uH2JI9qp7ZZMeJ80DQHI53FY9pmdsmyLbKsHoqZxuBvakFZ9qA2R',
		   );
	   $headers = array(
	...
	// qRRSPESqq4IbkHzA4swVms0zFNmMJb
});
