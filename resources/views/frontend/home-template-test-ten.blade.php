...
@section('css-links')
    ...
    <script>
        var featured_products_length =
            {{ isset($homePageData['featured_products']) ? count($homePageData['featured_products']) : '' }};
    </script>
@endsection
@section('css')
    <style>
...
@section('content')
    ...
    <!-- html code here -->
    <button type="button" class="btn btn-primary d-none" data-toggle="modal" data-target="#login_modal"> Launch demo
        modal </button>
    @if (count($banners))
        <section class="home-slider-wrapper pt-md-0 pb-0 banner_block">
            ...
            <div class="container">
                <div class="banner_row">
                                ...
                                @php
                                    ...
                                    if ($banner->link == 'category') {
                                        if (!empty($banner->category_slug)) {
                                            $url = route('categoryDetail', $banner->category_slug);
                                        }
                                    } elseif ($banner->link == 'vendor') {
                                        if (!empty($banner->vendor_slug)) {
                                            $url = route('vendorDetail', $banner->vendor_slug);
                                        }
                                    } elseif ($banner->link == 'url') {
                                ...
                                <div class="carousel-item @if ($key == 0) active @endif">
                                    <a class="banner-img-outer" href="{{ $url ?? '#' }}" target="_blank">
                                        <link rel="preload" as="image"
                                            href="{{ get_file_path($banner->image, 'IMG_URL1', '1170', '500') }}" />
                                        <img alt="" title="" class="blur-up lazyload w-100"
                                            data-src="{{ get_file_path($banner->image, 'IMG_URL1', '1170', '500') }}">
                                    </a>
                                </div>
                            @endforeach
                    ...
                    <div id="myMobileCarousel" class="carousel slide al_mobile_banner mb-2" data-ride="carousel"
                        style="display:none;">
                        <div class="carousel-inner">
                            ...
                            @foreach ($mobile_banners as $key => $banner)
                                ...
                                <div class="carousel-item @if ($key == 0) active @endif">
                                    <a class="banner-img-outer" href="{{ $url ?? '#' }}">
                                        <link rel="preload" as="image"
                                            href="{{ get_file_path($banner->image, 'IMG_URL1', '400', '150') }}" />
                                        <img alt="" title="" class="blur-up lazyload w-100"
                    ...
                    <div class="right_img">
                        <div class="image">
                            @php
                                $url = '';
                                if (@$rightSideBanner->link == 'category') {
                                    if (!empty($rightSideBanner->category->slug)) {
                                        $url = route('categoryDetail', @$rightSideBanner->category->slug);
                                    }
                                } elseif (@$rightSideBanner->link == 'vendor') {
                                    if (!empty(@$rightSideBanner->vendor->slug)) {
                                        $url = route('vendorDetail', $rightSideBanner->vendor->slug);
                                    }
                                }
                            @endphp
                            {{-- <img src="https://s3.us-west-2.amazonaws.com/royoorders2.0-assets/prods/61yXnXnJomckoenS3iZj4m3Qv7aCtUJHLxECPddS.jpg" alt=""> --}}
                            <a href="{{ $url ?? '#' }}"><img src="{{ @$rightSideBanner->image['original'] }}"
                                    ></a>
                        </div>
                    </div>
    ...
    <section class="no-store-wrapper mb-3 mt-3" style="display: none;">
        <div class="container">
            ...
            @else
                <div class="row">
                    <div class="col-12 text-center"> <img class="no-store-image mt-2 mb-2 blur-up lazyload"
                            data-src="{{ getImageUrl(asset('images/no-stores.svg'), '250/250') }}"
...
%>
    ...
    <section
        class="section-b-space ratio_asos pt-0 mt-0 pb-0 {{ isset($client_preference_detail) && $client_preference_detail->business_type == 'taxi' ? 'taxi' : '' }}"
        id="our_vendor_main_div">
        ...
        <a class="" href="">See All  <i class="fa fa-angle-right" aria-hidden="true"></i> </a>  -->
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12 brand text-left">
                                <div class=" top-heading text-left">
                                    <h2 class="h2-heading text-left">top-Brands</h2>
                                    <!-- <a class="" href="javscript:void(0);">See All <i class="fa fa-angle-right" aria-hidden="true"></i></a> -->
                                </div>
                            </div>

                            <div class=" col-md-12 al_custom_brand">
                                <div class="alNewSliderTopBrand render_{{ $homePageLabel->slug }}"
                                    id="{{ $homePageLabel->slug . $key }}">
                ...
                @elseif($homePageLabel->slug == 'cities' && count($homePageData['cities']) != 0)
                    <section class="suppliers-section container render_full_{{ $homePageLabel->slug }}">
                        <div class=" top-heading d-flex justify-content-between align-self-center">
                            <h2 class="col h2-heading">
                                {{ !empty($homePageLabel->translations->first()->title) ? $homePageLabel->translations->first()->title : 'Popular Areas' }}
                            </h2>
                        </div>
                        <div class="col-12">
                            <div class="suppliers-slider-{{ $homePageLabel->slug }} product-m renderqw_{{ $homePageLabel->slug }}"
                                id="{{ $homePageLabel->slug . $key }}">
                                @foreach ($homePageData[$homePageLabel->slug] as $cities)
                                    <div class="alSpaListSlider">
                                        <div>
                                            <div class="alSpaListBox">
                                                <div class="alSpaCityBox">
                                                    <a href="javascript:void(0);" class="cities updateLocationByCity"
                                                        data-lat="{{ $cities['latitude'] }}"
                                                        data-long="{{ $cities['longitude'] }}"
                                                        data-place_id="{{ $cities['place_id'] }}"
                                                        data-address="{{ $cities['address'] }}"><img class="w-100"
                                                            src="{{ $cities['image']['image_fit'] }}260/260{{ $cities['image']['image_path'] }}"></a>
                                                </div>
                                                <p>{{ $cities['title'] }} </p>
                                            </div>
                                        </div>
        ...
        <a class="" href="{{ route('vendor.all') }}">{{ __('See all') }} <img class="btn-arrow" src="{{ asset('images/template-8/arrow.png') }}" alt="" title=""></a>
                        ...
                        <div class="row eccomerce-temp">
                            <div class="col-md-2 text-center">
                                <div class=" top-heading mt-5 pt-5">
                                    <h2 class="h2-heading">
                                        {{ !empty($homePageLabel->translations->first()->title) ? $homePageLabel->translations->first()->title : getNomenclatureName('Recommended Vendor', true) }}
                                    </h2>
                                    <a class="" href="{{ route('vendor.all') }}">{{ __('See All') }} <i
                                            class="fa fa-angle-right" aria-hidden="true"></i></a>
                                </div>
                            </div>
                            <div class="col-md-10">
                                <div class="suppliers-slider-{{ $homePageLabel->slug }} product-m render_{{ $homePageLabel->slug }}"
                                    id="{{ $homePageLabel->slug . $key }}">
                ...
                @elseif($homePageLabel->slug == 'trending_vendors' && count($homePageData['trending_vendors']) != 0)
                    <section class="suppliers-section container" id="homepage_trending_vendors_div">
                        <div class=" top-heading ">
                            <h2 class="h2-heading">
                                {{ $homePageLabel->slug == 'trending_vendors' ? __('Trending') . ' ' . getNomenclatureName('vendors', true) : __($homePageLabel->title) }}
                            </h2>
                        </div>
                        <div class="row">
                ...
                @elseif($homePageLabel->slug == 'recent_orders' && count($homePageData['recent_orders']) != 0)
                    <section class="container best-seller-full mb-0 render_full_{{ $homePageLabel->slug }}"
                        id="{{ $homePageLabel->slug . $key }}">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="recent-orders-8 product-m  render_{{ $homePageLabel->slug }}"
                                    id="{{ $homePageLabel->slug . $key }}">
                ...
                @elseif($homePageLabel->slug == 'recently_viewed' && count($homePageData['recently_viewed']) != 0)
                    <section class="main-pro-slider p2p-full-width suppliers-section container mb-4"
                        id="homepage_recently_viewed">

                        <div class="row">
                            <div class="col-md-2 mt-5 view text-center">
                                <div class=" top-heading mt-4 pt-5">
                                    <h2 class="h2-heading">
                                        {{ $homePageLabel->slug == 'recently_viewed' ? __('Recently') . ' ' . getNomenclatureName('Viewed', true) : __($homePageLabel->title) }}
                                    </h2>
                                    <a class="" href="">See All <i class="fa fa-angle-right"
                            ...
                            <div class=" ">
                                <div class="">
                                    <div class=" top-heading text-center d-flex justify-content-between">
                                        <h2 class=" h2-heading">
                                              {{ ($homePageLabel->slug == 'top_rated' &&   isset($homePageLabel->translations->first()->title)) ?  @$homePageLabel->translations->first()->title  : __('Top')." ".getNomenclatureName('Rated', true) }}
                                        </h2>
                                        <a class="" href="{{route('getProducts',"top_rated")}}">View All <i class="fa fa-angle-right"
                                                aria-hidden="true"></i> </a>
                                                {{-- <a class="" href="javascript:void(0)">See All <i class="fa fa-angle-right"
                                                    aria-hidden="true"></i> </a> --}}
                                    </div>
                                </div>
                            </div>
                            <!-- suppliers-slider-{{ $homePageLabel->slug }} -->
                            <div class="product-m render_{{ $homePageLabel->slug }}"
                                id="{{ $homePageLabel->slug . $key }}">
                                <div class=" top_rated_slider">
                                    @foreach ($homePageData[$homePageLabel->slug] as $product)
                ...
                @elseif(
                    !empty($homePageLabel->nav_categories) &&
                        $homePageLabel->slug == 'nav_categories' &&
                        count($homePageLabel->nav_categories) != 0)
                    <section class="popular-brands left-shape_ position-relative bg-light-gray">
                        <div class="container">
                            ...
                            <div class="slider_row">
                                <div class="  al_custom_categories">
                                    <div class="render_{{ $homePageLabel->slug }}" id="{{ $homePageLabel->slug . $key }}">
                                        <div class=" categories_sliderEight">
                ...
                @elseif($homePageLabel->slug == 'spotlight_deals' && count($homePageData['spotlight_deals']) != 0)
                    <section class="mb-0 render_full_{{ $homePageLabel->slug }}" id="{{ $homePageLabel->slug . $key }}">
                        <div class="container full_width_eccomerce">
                            ...
                            <div class="product-4-{{ $homePageLabel->slug }} product-m  render_{{ $homePageLabel->slug }}"
                                id="{{ $homePageLabel->slug . $key }}">
                ...
                @elseif($homePageLabel->slug == 'best_sellers' && count($homePageData['best_sellers']) != 0)
                    <section class="container best-seller-eccomerce mb-0 render_full_{{ $homePageLabel->slug }}"
                        id="{{ $homePageLabel->slug . $key }}">
                        <div class="top-heading d-flex justify-content-between">
                            <h2 class="col h2-heading"> @php
                                echo !empty($homePageLabel->translations->first()->title) ? $homePageLabel->translations->first()->title : __($homePageLabel->title);
                ...
                @elseif($homePageLabel->slug == 'single_category_products' && count($homePageData[$homePageLabel->slug]) != 0)
                    <section class="main-product single_category_products mb-0 render_full_{{ $homePageLabel->slug }}"
                        id="{{ $homePageLabel->slug . $key }}">
                        <div class="container p2p-full-width">
                            <div class="top-heading d-flex justify-content-between">
...
@elseif(@$homePageLabel->slug == 'selected_products' && @count(@$homePageData[@$homePageLabel->slug]) != 0)
    <section class="main-product single_category_products mb-0 render_full_{{ $homePageLabel->slug }} bg-light-gray"
        id="{{ $homePageLabel->slug . $key }}">
        <div class="container p2p-full-width">
            <div class="top-heading d-flex justify-content-between">
                <h2 class="col h2-heading mb-3"> @php
                    echo !empty($homePageLabel->translations->first()->title) ? $homePageLabel->translations->first()->title : __($homePageLabel->title);
                @endphp </h2>
                <a class="" href="{{route('getProducts',"selected_products")}}">See All <i class="fa fa-angle-right" aria-hidden="true"></i> </a>
            </div>
            <div class="row">
                <div class="col-12">
                    <div class="product-4-{{ $homePageLabel->slug }} product-m  render_{{ $homePageLabel->slug }}"
                    ...
                    <div class="container">
                        <div class="text-center">
                            ...
                            @if (in_array($extension, $image_extensions))
                                <img alt="" title="" class="blur-up lazyload w-100"
                                    src="{{ $homePageData['banners'][$homePageLabel->translations->first()->cab_booking_layout_id] }}"
                                    height="300">
...
@else
                    ...
                    <div class="col-md-12 pl-0 text-center">
                        <div class="top-heading d-flex justify-content-between">
                            <h2 class="h2-heading"> @php
                                echo !empty($homePageLabel->translations->first()->title) ? $homePageLabel->translations->first()->title : __($homePageLabel->title);
                            @endphp </h2>
                            @if($homePageLabel->slug == "new_products")
                            <a class="" href="{{route('getProducts','new_products')}}">View All <i class="fa fa-angle-right" aria-hidden="true"></i> </a>
                            {{-- <a class="" href="javascript:void(0)">See All <i class="fa fa-angle-right" aria-hidden="true"></i> </a> --}}
                            @endif
                            @if($homePageLabel->slug == "featured_products")
                            <a class="" href="{{route('getProducts','featured_products')}}">View All <i class="fa fa-angle-right" aria-hidden="true"></i> </a>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="product-m  render_{{ $homePageLabel->slug }} {{ $homePageLabel->slug == 'on_sale' ? 'on_sale_product' : '' }}"
                    id="{{ $homePageLabel->slug . $key }}">
                    @php
                        if ($homePageLabel->slug == 'featured_products') {
                            $sliderClass = 'p2p_eccomerce_slider10';
                        } elseif ($homePageLabel->slug == 'on_sale') {
                            $sliderClass = 'on_sale_slider';
                        } else {
                            $sliderClass = 'p2p_eccomerce_slider';
                        }
                    @endphp
                    <div class="row  {{ $sliderClass }} ">
                        @foreach ($homePageData[$homePageLabel->slug] as $product)
    ...
    <div class="modal age-restriction fade" id="age_restriction" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body text-center"> <img style="height: 150px;" class="blur-up lazyload"
                        data-src="{{ getImageUrl(asset('assets/images/age-img.svg'), '150/150') }}" alt=""
                        title="">
                    <p class="mb-0 mt-3">
                        {{ $client_preference_detail ? $client_preference_detail->age_restriction_title : __('Are you 18 or older?') }}
                    </p>
