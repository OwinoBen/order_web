...
trait ProductActionTrait{
    ...
    public function getBannersForHomePage($client_preferences, $banner_type, $latitude, $longitude)
    {
        try
        {
            ...
            $mainQuery.= " $joinQuery WHERE `ba`.`status` =1 AND `ba`.`validity_on` = 1 AND (`ba`.`start_date_time` is null or (date(`ba`.`start_date_time`) <= '".$carbon_now."' and date(`ba`.`end_date_time`) >= '".$carbon_now."')) and `ba`.`name` != 'Right Side Banner'";
