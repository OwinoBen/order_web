    ...
    public function productsByVendorOptimize(Request $request, $vid = 0){
        try {
            ...
            /* $vendor_categories = Category::select('id')->whereHas('vendorCategory',function ($q)use($vid){
                ...
                foreach($vendor_categories as $category){
                    $category->data = Product::where('is_live', 1)->where('vendor_id', $vid)->where('category_id',$category->id)->with([
                    'inwishlist' => function($qry) use($userid){
                        $qry->where('user_id', $userid);
                    },
                    'media.image',
                    'translation' => function($q) use($langId){
                        $q->select('product_id', 'title', 'body_html', 'meta_title', 'meta_keyword', 'meta_description','language_id','body_html as translation_description')->where('language_id', $langId)->orderBy('body_html','desc')->groupBy('title');
                        $q->groupBy('language_id','product_id');
                    },
                    'variant' => function($q) use($langId, $multipli){
                        $q->select('id','sku', 'product_id', 'quantity', 'price', 'markup_price','barcode', 'compare_at_price',DB::raw("'$multipli' as  multiplier"))->orderBy('quantity', 'desc');
                    ...
                    'tags.tag.translations' => function ($q) use ($langId) {
                        $q->where('language_id', $langId);
                    }])->join('product_translations', 'product_translations.product_id', '=', 'products.id')->select('products.*',\DB::raw("'$multipli' as variant_multiplier"))->withCount(['variantSet','addOn'])
                    ->groupBy('products.id')->orderBy('product_translations.title', 'asc')->limit(15)->get();
                }
