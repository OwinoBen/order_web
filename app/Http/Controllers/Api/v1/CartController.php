    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            if (! empty($cartProduct)) {
                ...
                $total_minimum_spend =  \DB::selectOne("SELECT SUM(product_variants.price* cart_products.quantity) as max_price
               FROM cart_products left join
 product_variants ON cart_products.variant_id =
 product_variants.id WHERE cart_products.cart_id = ?", [$cartProduct->cart_id])->max_price;
                $total_addon_price = \DB::selectOne("SELECT SUM(addon_options.price* cart_products.quantity) as max_price FROM cart_products left join
 cart_addons ON cart_products.id =
 cart_addons.cart_product_id left join addon_options on addon_options.id = cart_addons.option_id where cart_products.cart_id = ?", [$cartProduct->cart_id])->max_price;
                $total_minimum_spend += !empty($total_addon_price)?$total_addon_price:0;
                $total_minimum_spend = $total_minimum_spend * $doller_compare;
                $cartData['cart_product_id'] = $cartProduct->id;
                $cartData['id'] = $cartProduct->cart_id;
                $cartData['item_count'] = CartProduct::where([
                    'cart_id' => $cartProduct->cart_id
                ])->sum('quantity');
                $cartData['gross_paybale_amount'] = $total_minimum_spend;
                return $this->successResponse($cartData);
            } else {
                return $this->successResponse($cartData);
            }
