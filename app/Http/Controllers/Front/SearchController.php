...
class SearchController extends FrontController
{
  ...
  /*  public function postAutocompleteSearch(Request $request)
    ...
    public function postAutocompleteSearch(Request $request)
    {
        ...
        $products = $products->whereNull('deleted_at')->groupBy('products.id')->get();
        if (count($products)) {
            $response = ['title' => '','type' =>'product', 'result' => $products];
        }
        if (count($vendors)) {
            $response = ['title' => __('Venders'),'type' =>'vendor', 'result' => $vendors];
        }
        ...
        if(count($brands)) {
            $response = ['title' => __('Brands'),'type' =>'brand', 'result' => $brands];
        }
        ...
        $categories = $categories->where(function ($q) use ($keyword) {
            ...
            ->orderBy('categories.position', 'asc')->get();
        if (count($categories)) {
            $response = ['title' => __('Categories'),'type' =>'category', 'result' => $categories];
        }
        $returnHTML = '';
        if(!empty($response)) {
            $returnHTML = view('layouts.store.search_result')->with(['title'=>$response['title'],'type' =>$response['type'],'result' => $response['result']])->render();
        }
        return response()->json(array('success' => true,'data'=>isset($response['result'])?$response['result']:'', 'html'=>$returnHTML));
    }
    ...
    public function showSearchResults($domain = "", $keyword)
    {
      ...
      // $preferences = Session::get('preferences');
        ...
        foreach ($products as $product) {
            $product->price = (($product->variant[0]->price ??  0.00) *  $clientCurrency->doller_compare);
            $redirect_url = route('productDetail', [$product->vendor_slug, $product->url_slug]);
            $image_url = $product->media->first() ? $product->media->first()->image->path['proxy_url'] . '300/300' . $product->media->first()->image->path['image_path'] : '';
            $product_results[] = ['id' => $product->id, 'name' => $product->dataname, 'latitude' => $product->latitude, 'longitude' => $product->longitude, 'address' => $product->address, 'image_url' => $image_url, 'redirect_url' => $redirect_url,'price'=>$product->price];
            if (!empty($vendorMapView)) {
