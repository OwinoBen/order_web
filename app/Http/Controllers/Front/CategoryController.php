...
use Redirect;
use Log;
use \App\Http\Traits\{ProductActionTrait, VendorTrait};
class CategoryController extends FrontController{
    private $field_status = 2;
    use \App\Http\Traits\DispatcherSlot,VendorTrait,ProductActionTrait;
        ...
        if($page == 'pickup/delivery'){
            ...
            if($page == 'laundry' || $service_type == 'rental_service')
                ...
                if(view()->exists('frontend/cate-'.$page.'s')){
                    return view('frontend/cate-'.$page.'s')->with(['maxPrice'=>$maxPrice,'listData' => $listData, 'category' => $category, 'navCategories' => $navCategories, 'newProducts' => $newProducts, 'variantSets' => $variantSets, 'productAttributes'=> $productAttributes, 'productsCount' => $productsCount]);
                }else{
...
//                     }
    ...
    public function getProducts(Request $request, $domain, $productType)
        {
            $preferences = Session::get('preferences');
            $langId = Session::get('customerLanguage');
            $navCategories = $this->categoryNav($langId);

            $additionalPreference = getAdditionalPreference(['is_admin_vendor_rating']);
            $latitude  =  Session::get('latitude');
            $longitude =  Session::get('longitude');
            $vendorIds = $this->getRandomVendorIdsForHomePage($preferences, "delivery", $additionalPreference['is_admin_vendor_rating'], $latitude, $longitude);
                  $productIds = $this->getProductsId("top_rated_products");
                    ...
                    if($productType == "new_products"){
                        $products = $products->where('is_new', 1)->whereIn('vendor_id', $vendorIds);
                    }elseif($productType == "top_rated"){
                        $products = $products->whereIn('id', $productIds);
                    }elseif($productType == "featured_products"){
                        $products = $products->where('is_featured', 1)->whereIn('vendor_id', $vendorIds);
                    }
                     $productCount = $products->count();
                     $products = $products->distinct()->paginate(12);
            return view('frontend.get-products')->with(['navCategories' => $navCategories,'slug' => $productType, 'products' => $products, 'productCount' => $productCount]);
        }
        ...
        public function filterProducts(Request $request)
        {
            ...
            $vendorIds = $this->getRandomVendorIdsForHomePage($preferences, "delivery", $additionalPreference['is_admin_vendor_rating'], $latitude, $longitude);
                  ...
                  $products = Product::with([
                    ...
                    ->select('products.id', 'products.sku', 'products.averageRating', 'products.url_slug', 'products.is_new', 'products.is_featured', 'products.vendor_id', 'products.inquiry_only','products.minimum_order_count','products.batch_count')
                    ...
                    $returnHTML = view('frontend.get-products-card')->with(['products' => $products])->render();
                    return response()->json(array('success' => true, 'html'=>$returnHTML));
        }
}
