    ...
    public function postForgotPassword(Request $request, $domain = ''){
        try {
            ...
            if (!empty($data->mail_driver) && !empty($data->mail_host) && !empty($data->mail_port) && !empty($data->mail_port) && !empty($data->mail_password) && !empty($data->mail_encryption)) {
                ...
                $token = Str::random(60);
                \Log::info(url('/reset-password/'.$token));
                ...
                if($email_template){
                    ...
                    $email_template_content = str_ireplace("{reset_link}", 'https://patazone.co.ke/reset-password/'.$token, $email_template_content);
                }
                \Log::info($email_template_content);
